...
async def create_transcript_job(data: TranscriptPayload, job_id: str):
    """Create a transcript job with pending status"""
    try:
        print(f"💾 Creating transcript job: {job_id}")
        ...
        print("✅ Transcript job created successfully")
        return response
    except Exception as e:
        print(f"❌ Error creating transcript job: {str(e)}")
        raise e

async def update_transcript_status(job_id: str, status: str, ai_feedback: str = None):
    """Update transcript job status"""
    try:
        print(f"🔄 Updating transcript job {job_id} to status: {status}")
        update_data = {"status": status}
        if ai_feedback:
            update_data["ai_feedback"] = ai_feedback
        
        response = supabase.table("transcripts").update(update_data).eq("job_id", job_id).execute()
        print(f"✅ Transcript job {job_id} updated successfully")
        return response
    except Exception as e:
        print(f"❌ Error updating transcript job: {str(e)}")
        raise e
